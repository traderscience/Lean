...
namespace QuantConnect.ToolBox.FredDownloader
{
    ...
    public class FredDataDownloader : IDataDownloader, IDisposable
    {
        ...
        private static IEnumerable<EconomicRelease> GetReleases(string apiKey)
        {
            IEnumerable<BaseData> data = new List<AuxiliaryData>();
            Frequency? frequency = null;
            IEnumerable<Release> rawData = null;

            try
            {
                var client = new Xaye.Fred.Fred(apiKey, true);
                rawData = client.GetReleases();
            ...
            {
                Log.Error($"FredReleasesDownloader:GetReleases: exception: {ex.Message}");
                yield break;
            }
            ...
            foreach (var item in rawData)
            {
                if (item != null)
                {
                    var dataPoint = ParseReleaseData(item);
                    if (dataPoint == null)
                        continue;
        ...
        private static IEnumerable<BaseData> GetEnumerator(string seriesCode, DateTime start, DateTime end, string apiKey)
        {
            ...
            foreach (var item in rawData)
            {
                if (item != null)
                {
                    var dataPoint = ParseAuxiliaryData(item);
                    if (dataPoint == null)
                        continue;
                    yield return dataPoint;
                }
        ...
        private static AuxiliaryData ParseAuxiliaryData(Observation data)
        {
        ...
        private static EconomicRelease ParseReleaseData(Release release)
        {
            if (release == null)
            {
                return null;
            }
            var ecoRelease = new EconomicRelease();
            ecoRelease.ReleaseTime = release.RealtimeStart;
            ecoRelease.Id = release.Id.ToStringInvariant();
            ecoRelease.Name = release.Name;
            ecoRelease.Url = release.Link;
            return ecoRelease;
        }
