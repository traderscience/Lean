...
namespace QuantConnect.ToolBox.QuandlDownloader
{
    ...
    /// </summary>
    public class QuandlDataDownloader : IDataDownloader
    {
        private string _apiKey;
        public QuandlDataDownloader(string apiKey)
        {
            _apiKey = apiKey;
        }
        ...
        private static IEnumerable<BaseData> GetEnumerator(string symbol, string datasetCode, DateTime start, DateTime end, string apiKey)
        {
            ...
            {
                Log.Error($"QuandlDataDownloader:GetEnumerator: exception: {ex.Message}");
                yield break;
            }
