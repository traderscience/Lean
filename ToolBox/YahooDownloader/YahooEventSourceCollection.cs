...
namespace QuantConnect.ToolBox.YahooDownloader
{
    ...
    /// </summary>
    public class YahooEventSourceCollection : IDisposable
    {
        ...
        /// <summary>
        /// Creates a new instance of <see cref="YahooEventSourceCollection"/>
        /// </summary>
        public YahooEventSourceCollection(EventHandler<MessageReceivedEventArgs> messageAction, string apiKey)
        {
        ...
        {
            Log.Debug("YahooEventSourceCollection.UpdateSubscription(): Subscription update started");
            ...
            foreach (var kvp in ClientSymbolsDictionary)
            {
                ...
                {
                    Log.Debug($"YahooEventSourceCollection.UpdateSubscription(): Leave unchanged subscription for: {string.Join(",", clientSymbols)}");
            ...
            Log.Debug($"YahooEventSourceCollection.UpdateSubscription(): {clientsToRemove.Count} old clients to remove");

            if (!remainingSymbols.Any())
            {
                throw new Exception("YahooEventSourceCollection.UpdateSubscription(): Invalid logic, remaining symbols can't be an empty set.");
            }
            ...
            {
                Log.Debug($"YahooEventSourceCollection.CreateNewSubscription(): Creating new subscription for: {string.Join(",", package)}");
            ...
            {
                throw new Exception("YahooEventSourceCollection.UpdateSubscription(): Could not update subscription within a timeout");
            }

            clientsToRemove.DoForEach(RemoveOldClient);

            // Reset counter
            Log.Debug($"YahooEventSourceCollection.CreateNewSubscription(): Updated successfully. Resetting the counter.");
            Counter.Reset(1);
        ...
        {
            Log.Debug($"YahooEventSourceCollection.UpdateSubscription(): Remove subscription for: {string.Join(",", ClientSymbolsDictionary[client])}");
        ...
        {
            var url = "https://finance.yahoo.com/stable/quotes/";
