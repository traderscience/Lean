...
namespace QuantConnect.Indicators
{
    ...
    public class ZigZag : Indicator, IIndicatorWarmUpPeriodProvider
    {
        /// <summary>
        ...
        private ZigZagPoint initialPivot = null;
        private ZigZagPoint lastPivot = null;
        private IndicatorDataPoint max = null;
        private IndicatorDataPoint min = null;
        ...
        protected override decimal ComputeNextValue(IndicatorDataPoint input)
        {
            if (initialPivot == null) // still looking for initial pivot
            {
                ...
                if (max == null)
                    max = input;
                if (input.Value / min.Value >= upThreshold+1)
                {
                    initialPivot = new ZigZagPoint(input.EndTime, input.Value, PEAK);
                    lastPivot = initialPivot;
                    currentTrend = DOWNTREND;
                    return currentTrend;
                }
                if (input.Value / max.Value <= downThreshold+1)
                {
                    initialPivot = new ZigZagPoint(input.EndTime, input.Value, VALLEY);
                    lastPivot = initialPivot;
            ...
            var r = input.Value / lastPivot.Value;
            ...
            if (currentTrend == UPTREND)
            {
                if (r >= upThreshold + 1)
                {
                    ...
                    addPivot(lastPivot);
                    lastPivot = new ZigZagPoint(input.Time, input.Value, PEAK);
                    currentTrend = DOWNTREND;
                }
                else
                if (input.Value < lastPivot.Value)
                {
                    // downtrend is extending, update current pivot
                    lastPivot.Time = input.Time;
                    lastPivot.Value = input.Value;
                }
            }
            else
            {
                if (r <= downThreshold + 1)
                {
                    ...
                    addPivot(lastPivot);
                    lastPivot = new ZigZagPoint(input.Time, input.Value, VALLEY);
                    currentTrend = UPTREND;
                }
                else
                if (input.Value > lastPivot.Value)
